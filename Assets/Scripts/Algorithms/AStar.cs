...
public class AStar : MonoBehaviour {
  ...
  private int DetermineGScore(Vector2Int neighbour, Vector2Int current) {
    ...
    Actor actor = GameManager.Get.GetActorAtLocation((Vector3Int)neighbour);
  ...
  private Node GetNode(Vector2Int position) {
    if (MapManager.Get.Nodes.ContainsKey(position)) {
      return MapManager.Get.Nodes[position];
    } else {
      Node node = new Node(position);
      MapManager.Get.Nodes.Add(position, node);
      return node;
    }
  }
